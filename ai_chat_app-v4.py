...
class AIChatApp(QMainWindow):
    ...
    def load_config(self):
        try:
            with open('api_configuration.yaml', 'r') as file:
                ...
                if 'Available_Models' not in config: # Ensure 'Available_Models' key exists, default if not
                    config['Available_Models'] = ["gpt-4o-mini", "o3-mini", "deepseek/deepseek-chat", "deepseek/deepseek-r1", "deepseek-reasoner", "deepseek-chat"]
                return config
    ...
    def save_config(self, api_url, api_key, model, system_prompt, temperature):
        ...
        if not api_key.strip():
            ...
            return False
        temperature_val = float(temperature) # Ensure temperature is a float
        if not 0.0 <= temperature_val <= 2.0: # Validate temperature range
            QMessageBox.warning(self, "Configuration Error", "Temperature must be between 0.0 and 2.0.")
            return False
        valid_models = ["gpt-4o-mini", "o3-mini", "deepseek/deepseek-chat", "deepseek/deepseek-r1", "deepseek-reasoner", "deepseek-chat"] # Same list as in combo box, or a more comprehensive one
        if model.strip() and model.strip() not in valid_models: # Optional: Check if model is in the valid list (if not empty)
            QMessageBox.warning(self, "Configuration Error", f"Model '{model}' is not in the list of suggested models. Please ensure it's a valid model for your API endpoint.")
        # Note: You might choose to just warn, or to prevent saving invalid models. Currently, it's a warning.
        # --- End of Validation ---
        ...
        config = {
            ...
            'Temperature': temperature_val # Store the validated float value
        }
        try:
    ...
    def show_config_dialog(self):
        ...
        models = ["gpt-4o-mini", "o3-mini", "deepseek/deepseek-chat", "deepseek/deepseek-r1", "deepseek-reasoner", "deepseek-chat"]
        ...
        temperature_slider.setRange(0, 200) # Range 0 to 2.00 (representing 0 to 2)
        temperature_slider.setValue(int((self.config.get('Temperature', 0.5) if self.config else 0.5) * 100)) # Keep scaling by 100 for slider integer values
        temperature_label = QLabel(f"Temperature ({temperature_slider.value() / 100.0:.2f})") # Display with 2 decimal places
        temperature_slider.valueChanged.connect(lambda value: temperature_label.setText(f"Temperature ({(value / 100.0):.2f})")) # Update label with 2 decimal places
        ...
        if file_name:
            try:
                with open(file_name, 'r') as file:
                    content = file.read()
                    preview_length = 500 # Limit preview to 500 characters
                    preview = content[:preview_length]
                    if len(content) > preview_length:
                        preview += "...\n[Preview limited to first 500 characters]" # Indicate preview limit
                    self.attached_file_content = content # Store full content
    ...
    def call_api(self, prompt):
        if not self.config:
            QMessageBox.warning(self, "API Error", "API configuration is missing. Please configure API settings.") # QMessageBox for config issues
            return
        if not self.openai_client:
            QMessageBox.warning(self, "API Error", "OpenAI client not initialized. Check API configuration.") # QMessageBox for client init issues
            return
        ...
        try:
            ...
            # QMessageBox.critical(self, "API Error", error_message) # Removed QMessageBox here
            self.display_message("AI", f"Error: {error_message}", QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss"), is_error=True) # In-chat error display
